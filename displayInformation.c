...
void display() {
	...
	nxtDisplayBigTextLine(4, "%d direction", SensorValue[compass]); // %d Is a placeholder where the variable after the comma replaces "%d"
	//nxtDisplayBigTextLine(4, "%d cm", SensorValue[sonar]); // Displays the distance the ultrasonic sensor detects
	//nxtDisplayBigTextLine(4, "%d time", time100[T1]); // Displays the stopwatch[T1] (there are 4 seperate stopwatches, 1,2,3,4) in milliseconds
